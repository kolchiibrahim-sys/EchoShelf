...
final class AuthService: NSObject {
    ...
    func signUp(name: String, email: String, password: String,
        ...
        Auth.auth().createUser(withEmail: email,
            ...
            changeRequest.commitChanges { _ in
                ...
                FirebaseManager.shared.createUserProfile(uid: user.uid,
                                                         name: name,
                                                         email: email)
                completion(.success(user))
            }
    ...
    func resetPassword(email: String, completion: @escaping (Result<Void,
                                                             Error>) -> Void) {
    ...
    func signInWithGoogle(
        ...
        completion: @escaping (Result<FirebaseAuth.User,
                               Error>) -> Void
    ) {
    ...
    private var appleCompletion: ((Result<FirebaseAuth.User,
                                   Error>) -> Void)?
...
extension AuthService: ASAuthorizationControllerDelegate {
    func authorizationController(
        ...
        guard
            ...
            let tokenString = String(data: tokenData,
                                     encoding: .utf8)
        else {
        ...
        Auth.auth().signIn(with: credential) { [weak self] result,
            error in
            if let error {
...
enum AuthError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .appleCredentialInvalid:
            return "Apple giriş məlumatları etibarsızdır."
        }
    }
}

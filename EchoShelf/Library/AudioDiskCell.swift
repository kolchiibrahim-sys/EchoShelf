...
final class AudioDiskCell: UICollectionViewCell {
    ...
    override init(frame: CGRect) {
        ...
        NSLayoutConstraint.activate([
            titleLabel.topAnchor.constraint(equalTo: contentView.topAnchor, constant: 108),
    ...
    private func buildDisk() {
        ...
        let size     = min(contentView.bounds.width, 96.0)
        let center   = CGPoint(x: contentView.bounds.midX, y: size / 2)
        let radius   = size / 2

        spinLayer.frame    = CGRect(x: center.x - radius, y: center.y - radius,
                                    width: size, height: size)
        spinLayer.anchorPoint = CGPoint(x: 0.5, y: 0.5)
        contentView.layer.insertSublayer(spinLayer, at: 0)

        let diskPath = UIBezierPath(arcCenter: CGPoint(x: radius, y: radius),
        ...
        while grooveRadius > radius * 0.38 {
            let gl = CAShapeLayer()
            let gp = UIBezierPath(arcCenter: CGPoint(x: radius, y: radius),
                                   radius: grooveRadius, startAngle: 0,
                                   endAngle: .pi * 2, clockwise: true)
            gl.path        = gp.cgPath
        ...
        let labelPath   = UIBezierPath(arcCenter: CGPoint(x: radius, y: radius),
                                        radius: labelRadius, startAngle: 0,
                                        endAngle: .pi * 2, clockwise: true)
        ...
        let imgCenter  = CGPoint(x: radius, y: radius)
        ...
        NSLayoutConstraint.activate([
            coverImageView.centerXAnchor.constraint(equalTo: contentView.leadingAnchor, constant: center.x),
            coverImageView.centerYAnchor.constraint(equalTo: contentView.topAnchor, constant: center.y),
        ...
        let holePath   = UIBezierPath(arcCenter: CGPoint(x: radius, y: radius),
                                       radius: holeRadius, startAngle: 0,
                                       endAngle: .pi * 2, clockwise: true)

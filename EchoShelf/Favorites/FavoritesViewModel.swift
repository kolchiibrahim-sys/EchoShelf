...
final class FavoritesViewModel {
    ...
    func toggleKidsBook(_ ebook: Ebook) {
        ...
        } else { favoriteKidsBooks.insert(ebook, at: 0) }
        saveToStorage(); onDataUpdated?()
    }
    ...
    func toggleAuthor(_ author: Author) {
        ...
        else { favoriteAuthors.insert(author, at: 0) }
        saveToStorage(); onDataUpdated?()
    }

    func toggleGenre(_ genre: String) {
        if let idx = favoriteGenres.firstIndex(of: genre) { favoriteGenres.remove(at: idx) }
        else { favoriteGenres.insert(genre, at: 0) }
        saveToStorage(); onDataUpdated?()
    }
    ...
    func isAuthorFavorited(_ author: Author) -> Bool {
        favoriteAuthors.contains(where: { $0.firstName == author.firstName && $0.lastName == author.lastName })
    }
